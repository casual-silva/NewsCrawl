...
class ContentRules:
    ...
    def extract(self, text, host='', body_xpath='', noise_node_list=None, with_body_html=False):
        '''
        对 HTML 进行预处理可能会破坏 HTML 原有的结构，导致根据原始 HTML 编写的 XPath 不可用
        因此，如果指定了 title_xpath/author_xpath/publish_time_xpath，那么需要先提取再进行
        :param text: 网页文本
        :param host: 网址前缀
        :param body_xpath: 内容xpath规则
        :param noise_node_list: 噪声节点列表
        :param with_body_html: html源码
        :return: 正文
        '''
        ...
        content = self._extract(element, host=host, with_body_html=with_body_html, body_xpath=body_xpath)
        return content[0][1]['text']

    def _extract(self, selector, host='', body_xpath='', with_body_html=False):
