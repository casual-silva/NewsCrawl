...
class CsindexComCnSpider(scrapy.Spider):
    ...
    def parse(self, response):
        payload={"lang": "cn",
                 "page": {"key": "","page": str(response.meta['num']),"rows": 10},
                 "classList": [],
                 "indexList": [],
                 "relatedTopics": [],
                 "typeList": []
    ...
    def parse_csin(self, response):
        ...
        for d in data:
            ...
            pagetime=date2time(date_str=publishDate)
            url=f"https://www.csindex.com.cn/csindex-home/announcement/queryAnnouncementById?id={id}"
            yield from over_page(url, response, page_time=pagetime, callback=self.parse_detail,headers=self.headers)
        yield from over_page(response.url, response, page_time=pagetime,callback=self.parse,dont_filter=True)
