...
class CfbondComSpider(scrapy.Spider):
    ...
    def parse_news(self, response):
        ...
        # 翻页
        response.meta['num']+=1
        url = f"http://auto.cfbond.com/in/news/index_{response.meta['num']}.shtml"
        yield from over_page(url, response, page_num=response.meta['num']-2, callback=self.parse_news)
