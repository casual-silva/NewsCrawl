...
class P5wNetSpider(scrapy.Spider):
    name = 'p5w.net'
    ...
    # 分析链接页面之间相似性 分组抓取
    start_urls = [
        ["企业舆情", "首页 > 资讯>公司", "https://www.p5w.net/roll/company/"]
    ]
    ...
    def parse(self, response):
        # 详情页
        for url in response.css("p.item-title a"):
    ...
    def parse_detail(self, response):
        ...
        content=response.xpath('//*[@class="content-text"]//p//text()').getall()
        content_text=[x.strip() for x in content if x.strip() != '']
        item.add_value('content_text',content_text)  # 正文内容/text_content
        # 自定义规则
        item.add_xpath('article_source', '//*[@class="content-source"]/text()')  # 来源/article_source
