...
class ShoudianBjxComCnSpider(scrapy.Spider):
    name = 'shoudian.bjx.com.cn'
    ...
    # 分析链接页面之间相似性 分组抓取
    start_urls = [
        ["企业舆情", "首页 > 企业", "https://shoudian.bjx.com.cn/NewsList?id=76"]
    ]
    ...
    def parse_jijing(self, response):
        ...
        next_url = f"https://shoudian.bjx.com.cn{page}"
        response.meta['num'] += 1
    ...
    def parse_detail(self, response):
        ...
        content_ = response.xpath('//*[@class="newsrand"]/p//text()').getall()
        content__ = response.xpath('//*[@class="content"]/p//text()').getall()
        content_1 = response.xpath('//*[@class="content"]/div[not(@class)]/p//text()').getall()
        contenx=content_rules.extract(response.text)
        for c in [content,content_,content__,content_1]:
            if c != []:
                content_text = c
            else:
                content_text = contenx
