...
class UchekeJrjComCnSpider(scrapy.Spider):
    name = 'ucheke.jrj.com.cn'
    allowed_domains = ['ucheke.jrj.com.cn']
    site_name = '金融界汽车'
    ...
    # 分析链接页面之间相似性 分组抓取
    start_urls = [
        ["企业舆情", "首页>滚动", "http://ucheke.jrj.com.cn/list/n_gundong_list.shtml"]
    ]
    ...
    def parse_jijing(self, response):
        for url in response.css(".ull li label a"):
            yield response.follow(url, callback=self.parse_detail, meta=response.meta)

        # 翻页
        page=response.xpath('//a[text()="下一页"]/@href').get()
        date_list=response.xpath('//*[@class="ull"]/li/text()').getall()
        page_time=date2time(date_str=date_list[-1])
        next_url=f"http://ucheke.jrj.com.cn/list{page}"
        response.meta['num'] += 1
    ...
    def parse_detail(self, response):
        ...
        item.add_xpath('title','//*[@class="titmain"]/h1/p/text()')  # 标题/title
        item.add_value('publish_date', self.publish_date_rules.extractor(response.text))  # 发布日期/publish_date
        item.add_value('content_text', content_rules.extract(response.text))  # 正文内容/text_content
        # 自定义规则
        item.add_xpath('article_source', '//*[@class="inftop"]/span/i/a/text()')  # 来源/article_source
