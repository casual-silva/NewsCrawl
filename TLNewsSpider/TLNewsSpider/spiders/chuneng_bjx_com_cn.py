...
class ChunengBjxComCnSpider(scrapy.Spider):
    name = 'chuneng.bjx.com.cn'
    allowed_domains = ['chuneng.bjx.com.cn']
    site_name = '北极星储能网'
    ...
    # 分析链接页面之间相似性 分组抓取
    start_urls = [
        ["行业舆情", "北极星储能网 > 企业", "https://chuneng.bjx.com.cn/NewsList?id=76"],
        ["企业舆情", "北极星储能网 > 要闻", "https://chuneng.bjx.com.cn/NewsList/"]
    ]
    ...
    def parse_jijing(self, response):
        for url in response.css(".list_content ul li a"):
            yield response.follow(url, callback=self.parse_detail, meta=response.meta)
    
        # 翻页
        page = response.xpath('//a[@title="下一页"]/@href').get()
        next_url = f"https://chuneng.bjx.com.cn{page}"
        response.meta['num'] += 1
    ...
    def parse_detail(self, response):
        ...
        content = response.xpath('//*[@class="list_detail"]/div[not(@class)]/p//text()').getall()
        # content_ = response.xpath('//*[@class="hydetail_content"]/div[not(@class)]/p//text()').getall()
        content_ = response.xpath('//*[@class="list_detail"]/p//text()').getall()
