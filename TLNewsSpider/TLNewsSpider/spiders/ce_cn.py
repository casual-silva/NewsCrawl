...
class CeCnSpider(scrapy.Spider):
    ...
    def parse_normal(self, response):
        ...
        for i in range(1, 20):
            page = f"{response.meta['url']}index_{i}.shtml"
            # yield response.follow(page, meta=response.meta)
            yield from over_page(page, response, page_num=i, callback=self.parse_normal)

    def parse_fc(self, response):
        for url in response.css(".sec_left .con li a"):
            yield response.follow(url, callback=self.parse_detail, meta=response.meta)

        # 翻页
        for i in range(1,20):
            page=f"{response.meta['url']}index_{i}.shtml"
            yield from over_page(page, response, page_num=i, callback=self.parse_fc)
            # yield response.follow(page, meta=response.meta)
    ...
    def parse_hydt(self, response):
        ...
        for i in range(1, 20):
            page = f"{response.meta['url']}index_{i}.shtml"
            yield from over_page(page, response, page_num=i, callback=self.parse_hydt)
            # yield response.follow(page, meta=response.meta)
