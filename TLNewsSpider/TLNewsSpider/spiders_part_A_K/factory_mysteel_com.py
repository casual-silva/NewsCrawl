...
class FactoryMysteelComSpider(scrapy.Spider):
    name = 'factory.mysteel.com'
    ...
    # 分析链接页面之间相似性 分组抓取
    start_urls = [
        ["企业舆情", "钢厂频道 > 钢厂动态 > 上市公司 > 文章列表", "https://list1.mysteel.com/article/p-382-------------1.html"],
    ...
    def parse_jijing(self, response):
        for url in response.css(".nlist li a"):
            yield response.follow(url, callback=self.parse_detail, meta=response.meta)

        # 翻页
        page=response.xpath('//a[text()="下一页"]/@href').get()
        date_list=response.xpath('//*[@class="date"]/text()').getall()
        page_time=date2time(min_str=date_list[-1])
        next_url=f"https://list1.mysteel.com{page}"
        response.meta['num'] += 1
    ...
    def parse_detail(self, response):
        ...
        item.add_xpath('content_text', '//*[@class="text"]/p[not(@style)]//text()')  # 正文内容/text_content
        # 自定义规则
