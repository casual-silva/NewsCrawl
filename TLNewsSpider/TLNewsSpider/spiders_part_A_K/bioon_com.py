...
class BioonComSpider(scrapy.Spider):
    ...
    def parse(self, response):
        # 详情页
        for data in response.xpath('//*[@class="item-content"]'):
            data_url=data.xpath('./h2/a/@href').get()
            data_time=data.xpath('.//*[@class="item-meta-item"]/text()').get()
            pagetime=date2time(date_str=data_time.strip())
            yield from over_page(data_url,response,page_time=pagetime)
