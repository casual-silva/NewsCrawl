...
class CbtComCnSpider(scrapy.Spider):
    name = 'cbt.com.cn'
    allowed_domains = ['cbt.com.cn']
    site_name = '中华工商网'
    ...
    # 分析链接页面之间相似性 分组抓取
    start_urls = [
        ["企业舆情", "中国工商网 > 公司 > 民企500强", "http://www.cbt.com.cn/gs/mq500q/"],
        ["企业舆情", "中国工商网 > 公司 > 中小企业", "http://www.cbt.com.cn/gs/zxqy/"],
        ["企业舆情", "中国工商网 > 公司 > 国有企业", "http://www.cbt.com.cn/gs/gq/"]
    ]
    ...
    def parse(self, response):
        # 详情页
        for data in response.xpath('//*[@class="list"]'):
            data_url=data.xpath('./a/@href').get()
            data_date=data.xpath('.//*[@class="time"]/text()').get()
            url=data_url.replace('./',response.meta['url'])
            pagetime=date2time(date_str=data_date)
    ...
    def parse_detail(self, response):
        ...
        # 自定义规则
        item.add_xpath('article_source', '//*[@class="time"]/span[2]/text()',re='来源：(.*)')  # 来源/article_source
