...
class ChyxxComSpider(scrapy.Spider):
    name = 'chyxx.com'
    allowed_domains = ['chyxx.com']
    site_name = '产业信息网'
    ...
    # 分析链接页面之间相似性 分组抓取
    start_urls = [
        ["行业舆情", "产业信息网 > 新闻中心 > 头条", "https://www.chyxx.com/news/topnews/"],
        ["行业舆情", "产业信息网 > 新闻中心 > 财经研究", "https://www.chyxx.com/news/caijing/"]
    ]
    ...
    def parse(self, response):
        for data in response.xpath('//*[@class="cx-media"]'):
            data_url=data.xpath('.//h3/a/@href').get()
            data_time=data.xpath('.//span/text()').get()
            url=f"https://www.chyxx.com{data_url}"
    ...
    def parse_detail(self, response):
        ...
        # 自定义规则
        item.add_xpath('article_source','//*[@class="article-title"]/label/text()', re='转自(.*)\xa0')  # 来源/article_source
